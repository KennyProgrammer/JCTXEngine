 ...
 * represents the orthographic projection area for the shadow render pass). It is updated each
 * frame to optimise the area, making it a small as possible (to allow for optimal shadow map 
 * resolution) while not being too small to avoid objects not having shadows when they should.
 * Everything inside the cuboidal area represented by this object will be rendered to the shadow 
 * map in the shadow render pass. Everything outside the area won't be.
 */
public class ShadowBox 
{
	 ...
	 */
	public ShadowBox(Matrix4f lightViewMatrix, EntityCamera camera) {
	...
	public void update() 
	{
		...
		toNear.scale(WorldRenderer.NEAR_PLANE);
	...
	{
		farWidth = (float) (SHADOW_DISTANCE * Math.tan(Math.toRadians(WorldRenderer.FOV)));
		nearWidth = (float) (WorldRenderer.NEAR_PLANE
				* Math.tan(Math.toRadians(WorldRenderer.FOV)));
